...
import { ProductCardSkeleton } from "../product-section-skeleton";
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products
      .filter((product) => {
        ...
        const matchesAttributes = Object.entries(activeFilters).every(
          ([groupId, values]) => {
            if (values.length === 0) return true;
            const attr = product.attributes?.find(
              (a) => a.name.toLowerCase() === groupId.toLowerCase()
            );
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-4">
                    ...
                    >
                      <AccordionItem value={cat.slug} className="border-none">
                        <div className="flex items-center gap-2 px-2">
                          ...
                          <AccordionTrigger className="py-2 hover:no-underline [&[data-state=open]>svg]:rotate-180"></AccordionTrigger>
                        </div>
                        <AccordionContent>
        ...
        <div className="lg:col-span-3 flex flex-col gap-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b pb-6">
            <div className="flex flex-wrap items-center gap-2 lg:hidden overflow-x-auto md:overflow-visible scrollbar-hide">
              <Badge
      ...
      {products.map((product) => (
        <ProductCard key={product.id} product={product} />
            ...
            <div className="flex min-h-[400px] flex-col items-center justify-center gap-4">
              ...
              <Button onClick={handleResetAll}>Clear Filters</Button>
            </div>
          )}
        </div>
      </div>
